...
namespace DevFast.Net.Extensions.Tests.SystemTypes
{
    ...
    public class ReadOnlySpanTests
    {
        ...
        [TestCase("", default(bool), false)]
        public void TryTo_Bool_On_SpanChar_Works_As_Expected(string input, bool expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out bool outcome), Is.EqualTo(returnVal));
        ...
        [TestCase(" c50bfbfe-d319-40e8-9998-33eb35b74b87  ", "c50bfbfed31940e8999833eb35b74b87", true)]
        [TestCase("  C50BFBFE-D319-40E8-9998-33EB35B74B87   ", "c50bfbfed31940e8999833eb35b74b87", true)]
        public void TryTo_Guid_On_SpanChar_Works_As_Expected(string input, string expected, bool returnVal)
        {
            Multiple(() =>
            {
                bool success = input.AsSpan().TryTo(out Guid outcome);
        ...
        public void TryTo_Int_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            int expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out int outcome, style,
        ...
        public void TryTo_Long_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            long expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out long outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome.Equals(expected));
            });
        }
        ...
        public void TryTo_Byte_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            byte expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out byte outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        public void TryTo_SByte_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            sbyte expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out sbyte outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        public void TryTo_Short_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            short expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out short outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        public void TryTo_UShort_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            ushort expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out ushort outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        public void TryTo_UInt_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            uint expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out uint outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        public void TryTo_ULong_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            ulong expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out ulong outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        [TestCase("3,40282", NumberStyles.Any, "fr-FR", (float)3.40282, true)]
        public void TryTo_Float_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            float expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out float outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        [TestCase("3,40282", NumberStyles.Any, "fr-FR", 3.40282, true)]
        public void TryTo_Double_On_SpanChar_Works_As_Expected(string input, NumberStyles style, string? provider,
            double expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out double outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
        ...
        public void TryTo_Decimal_On_SpanChar_When_Return_Value_Is_False(string input, NumberStyles style,
            string? provider)
        {
            Multiple(() =>
            {
                That(input.AsSpan().TryTo(out decimal outcome, style, string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.False);
                That(outcome, Is.EqualTo(0));
            });
        }

        [Test]
        public void TryTo_Decimal_On_SpanChar_When_Return_Value_Is_True()
        {
            Multiple(() =>
            {
                That("3.0001".AsSpan().TryTo(out decimal outcome), Is.True);
                That(outcome, Is.EqualTo(3.0001m));
                That("3.0000".AsSpan().TryTo(out outcome), Is.True);
                That(outcome, Is.EqualTo(3.0000m));
                That("3.40282".AsSpan().TryTo(out outcome), Is.True);
                That(outcome, Is.EqualTo(3.40282m));
                That("-3.40282".AsSpan().TryTo(out outcome), Is.True);
                That(outcome, Is.EqualTo(-3.40282m));
                That("3,40282".AsSpan().TryTo(out outcome, NumberStyles.Any, new CultureInfo("fr-FR")), Is.True);
                That(outcome, Is.EqualTo(3.40282m));
                That("-3,40282".AsSpan().TryTo(out outcome, NumberStyles.Any, new CultureInfo("fr-FR")), Is.True);
                That(outcome, Is.EqualTo(-3.40282m));
            });
            ...
            bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out DateOnly ts, format), Is.True);
                That(new DateOnly(2001, 01, 01), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out DateOnly ts, format), Is.False);
            ...
            string formatBarSeparated, bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out DateOnly ts, formatBarSeparated.Split('|')), Is.True);
                That(new DateOnly(2001, 01, 01), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out DateOnly ts, formatBarSeparated.Split('|')), Is.False);
                That(default(DateOnly), Is.EqualTo(ts));
            }
        }
        ...
        public void TryTo_DateOnly_Works_As_Expected_With_No_Format(string input, bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out DateOnly ts), Is.True);
                That(DateOnly.Parse(input), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out DateOnly ts), Is.False);
                That(default(DateOnly), Is.EqualTo(ts));
            }
        }
            ...
            bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out TimeOnly ts, format), Is.True);
                That(new TimeOnly(0, 0, 0), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out TimeOnly ts, format), Is.False);
            ...
            string formatBarSeparated, bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out TimeOnly ts, formatBarSeparated.Split('|')), Is.True);
                That(new TimeOnly(0, 0, 0), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out TimeOnly ts, formatBarSeparated.Split('|')), Is.False);
                That(default(TimeOnly), Is.EqualTo(ts));
            }
        }
        ...
        public void TryTo_TimeOnly_Works_As_Expected_With_No_Format(string input, bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out TimeOnly ts), Is.True);
                That(TimeOnly.Parse(input), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out TimeOnly ts), Is.False);
                That(default(TimeOnly), Is.EqualTo(ts));
            }
        }
            ...
            bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out DateTime ts, format), Is.True);
                That(new DateTime(2001, 01, 01, 0, 0, 0), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out DateTime ts, format), Is.False);
            ...
            string formatBarSeparated, bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out DateTime ts, formatBarSeparated.Split('|')), Is.True);
                That(new DateTime(2001, 01, 01, 0, 0, 0), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out DateTime ts, formatBarSeparated.Split('|')), Is.False);
                That(default(DateTime), Is.EqualTo(ts));
            }
        }
        ...
        public void TryTo_DateTime_Works_As_Expected_With_No_Format(string input, bool expectedReturn)
        {
            if (expectedReturn)
            {
                That(input.AsSpan().TryTo(out DateTime ts), Is.True);
                That(DateTime.Parse(input), Is.EqualTo(ts));
            }
            else
            {
                That(input.AsSpan().TryTo(out DateTime ts), Is.False);
                That(default(DateTime), Is.EqualTo(ts));
            }
        }
            ...
            int expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out int outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            long expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out long outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome.Equals(expected));
            });
        }
            ...
            byte expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out byte outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            sbyte expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out sbyte outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            short expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out short outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            ushort expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out ushort outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            uint expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out uint outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            ulong expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out ulong outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            float expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out float outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            double expected, bool returnVal)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out double outcome, style,
                    string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.EqualTo(returnVal));
                That(outcome, Is.EqualTo(expected));
            });
        }
            ...
            string? provider)
        {
            Multiple(() =>
            {
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out decimal outcome, style, string.IsNullOrWhiteSpace(provider) ? null : new CultureInfo(provider)), Is.False);
                That(outcome, Is.EqualTo(0));
            });
        }
        ...
        public void TryTo_Decimal_When_Return_Value_Is_True()
        {
            ...
            {
                string input = "3.0001";
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out decimal outcome), Is.True);
                That(outcome, Is.EqualTo(3.0001m));
                input = "3.0000";
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out outcome), Is.True);
                That(outcome, Is.EqualTo(3.0000m));
                input = "3.40282";
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out outcome), Is.True);
                That(outcome, Is.EqualTo(3.40282m));
                ...
                That(outcome, Is.EqualTo(-3.40282m));
                input = "3,40282";
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out outcome, NumberStyles.Any, new CultureInfo("fr-FR")), Is.True);
                That(outcome, Is.EqualTo(3.40282m));
                input = "-3,40282";
                That(new ReadOnlySpan<byte>(input.Select(x => (byte)x).ToArray()).TryTo(out outcome, NumberStyles.Any, new CultureInfo("fr-FR")), Is.True);
                That(outcome, Is.EqualTo(-3.40282m));
